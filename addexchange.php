<?php 
  ...
  if(isset($_POST['add_ex'])){
    ...
    if (empty($model) || empty($body) || empty($engine) || empty($hp) || empty($transmission) || empty($Engine_size) || /*empty($sunroof) || empty($wheels) ||*/ empty($steering_system)|| empty($airbag)|| empty($brake_system) || empty($A_C) || empty($aid_kit) || empty($fire) || empty($km) || empty($ex_with)) {
      $sta = "<div class='alert alert-danger'>All fields are required</div>";
    } else {
      $table = "cars";

      $colums = "`user_id`,`model`,`body`,`engine`,`hp`,`transmission`,`Engine_size`,`steering_system`,`airbag`,`brake_system`,`A_C`,`aid_kit`,`fire`,`km`,`ex_with`,`images`,`type`";

      $data = "'$userId','$model','$body','$engine','$hp','$transmission','$Engine_size','$steering_system','$airbag','$brake_system','$A_C','$aid_kit','$fire','$km','$ex_with','$nameImage','$type'";

      if (insert($table,$colums,$data)){
        $sta = "<div class='alert alert-success'>Exchange added</div>";
      } else {
          ...
          <form action="" class="row" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group">
                ...
                <select name="ex_with" class="form-control" id="" required>
                  <option value="same model higher class">Same model higher class</option>
                  <option value="same model lower class">Same model lower class</option>
                  <option value="different model higher class">Different model higher class</option>
                  <option value="different model lower class">Different model lower class</option>
                </select>
              </div>
              </div>
