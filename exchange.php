    ...
    <div class="row">
      <div class="col-md-4">
        ...
        <div class="custom mt-2">
          <a href="<?= url('exchange.php?sort=A_C')?>">A/C</a>
          <br>
          <a href="<?= url('exchange.php?sort=engine_size')?>">Engine size</a>
          <br>
          <a href="<?= url('exchange.php?sort=hp')?>">HP</a>
          <br>
          <a href="<?= url('exchange.php?sort=km')?>">Km</a>
        </div>
      </div>
      <div class="col-md-8">
        <div class="customt mb-2">
          <h6>Available cars for exchange:</h6>
        </div>
        <?php
          if (isset($_GET['sort'])) {
            $sort = $_GET['sort'];
            
            $new = select("cars","`type`='exchange' ORDER BY `cars`.`$sort` DESC");
          } else {
            $new = select("cars","`type`='exchange' ORDER BY `cars`.`id` DESC");
          }
          if ($new->rowCount() < 1) {
            echo "<div class='alert alert-danger'>Now cars available for exchange</div>";
          } else {
        ...
        <div class="custom mb-4 cars">
          <div class="row">
            ...
            <div class="col-md-6 mt-4">
              ...
              <br><b>Exchange with:</b> <?= $item['ex_with']?>
              <br><b>Images: </b>
              <?php 
